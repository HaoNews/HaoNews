...
type WriterPolicy struct {
	...
	// v0.3: trust anchors and per-channel unsigned
	TrustAnchors         []string          `json:"trust_anchors,omitempty"`          // trusted author URIs
	PerChannelUnsigned   map[string]bool   `json:"per_channel_unsigned,omitempty"`   // channel -> allow unsigned
	// v0.5: HD parent-child trust
	TrustMode            string            `json:"trust_mode,omitempty"`             // "exact" or "parent_and_children"
	VerifyDerivation     bool              `json:"verify_derivation,omitempty"`      // verify HD derivation path
}
...
// v0.5: acceptsOriginWithMessage checks policy with full message context (for HD parent extraction)
func (p WriterPolicy) acceptsOriginWithMessage(msg Message, scope string, store DelegationStore) bool {
	p.normalize()
	if isUnsignedOrigin(msg.Origin) {
		return p.AllowUnsigned
	}

	decision := p.originDecisionWithMessage(msg, scope, store)
	if decision.Capability == WriterCapabilityBlocked {
		return false
	}
...
// v0.5: originDecisionWithMessage includes HD parent from message extensions
func (p WriterPolicy) originDecisionWithMessage(msg Message, scope string, store DelegationStore) WriterOriginDecision {
	origin := msg.Origin
	p.normalize()
	if isUnsignedOrigin(origin) {
		if !p.AllowUnsigned {
	...
	// v0.3: check trust_anchors first
	if p.isTrustAnchor(origin.Author) {
	...
	// v0.5: check blacklist first (blacklist has priority)
	if p.isBlacklisted(origin.Author, origin.PublicKey) {
		return WriterOriginDecision{Capability: WriterCapabilityBlocked}
	}

	// v0.5: check HD parent-child trust from message extensions
	if p.TrustMode == "parent_and_children" {
		hdParent := getHDParentFromMessage(msg)
		if hdParent == "" {
			// Fallback: extract from author URI
			hdParent = p.getHDParent(origin)
		}
		if hdParent != "" {
			// Check if parent is blacklisted
			if p.isBlacklistedAuthor(hdParent) {
				return WriterOriginDecision{Capability: WriterCapabilityBlocked}
			}
			// Check if parent is in trust_anchors or whitelist
			if p.isTrustAnchor(hdParent) || p.isWhitelistedAuthor(hdParent) {
				return WriterOriginDecision{
					Capability:        WriterCapabilityReadWrite,
					ExplicitReadWrite: true,
				}
...
func (p WriterPolicy) originDecision(origin *MessageOrigin, scope string, store DelegationStore) WriterOriginDecision {
	...
	// v0.3: check trust_anchors first (always allow unsigned from trusted authors)
	if p.isTrustAnchor(origin.Author) {
		return WriterOriginDecision{
			Capability:        WriterCapabilityReadWrite,
			ExplicitReadWrite: true,
	...
	// v0.5: check HD parent-child trust
	if p.TrustMode == "parent_and_children" {
		if hdParent := p.getHDParent(origin); hdParent != "" {
			// Check if parent is blacklisted
			if p.isBlacklistedAuthor(hdParent) {
				return WriterOriginDecision{Capability: WriterCapabilityBlocked}
			}
...
// v0.3: check if author is in trust_anchors
func (p WriterPolicy) isTrustAnchor(author string) bool {
	author = normalizeFoldKey(author)
	for _, anchor := range p.TrustAnchors {
		if normalizeFoldKey(anchor) == author {
			return true
		}
	}
	return false
...
// v0.3: check if channel allows unsigned messages
func (p WriterPolicy) channelAllowsUnsigned(channel string) bool {
	channel = normalizeFoldKey(channel)
	if allow, ok := p.PerChannelUnsigned[channel]; ok {
		return allow
	}
	return p.AllowUnsigned
}

// v0.5: check if author or public key is blacklisted
func (p WriterPolicy) isBlacklisted(author, publicKey string) bool {
	return p.isBlacklistedAuthor(author) || p.isBlacklistedPublicKey(publicKey)
}

// v0.5: check if author is blacklisted
func (p WriterPolicy) isBlacklistedAuthor(author string) bool {
	author = normalizeFoldKey(author)
	for _, blocked := range p.BlockedAgentIDs {
		if normalizeFoldKey(blocked) == author {
			return true
		}
	}
	return false
...
// v0.5: check if public key is blacklisted
func (p WriterPolicy) isBlacklistedPublicKey(publicKey string) bool {
	publicKey = strings.ToLower(strings.TrimSpace(publicKey))
	for _, blocked := range p.BlockedPublicKeys {
		if strings.ToLower(strings.TrimSpace(blocked)) == publicKey {
			return true
		}
	}
	return false
...
// v0.5: check if author is whitelisted
func (p WriterPolicy) isWhitelistedAuthor(author string) bool {
	author = normalizeFoldKey(author)
	for _, allowed := range p.AllowedAgentIDs {
		if normalizeFoldKey(allowed) == author {
			return true
		}
	}
	// Check agent_capabilities
	if capability, ok := p.AgentCapabilities[author]; ok {
		return capability == WriterCapabilityReadWrite
	}
	return false
}

// v0.5: extract HD parent from message origin extensions
func (p WriterPolicy) getHDParent(origin *MessageOrigin) string {
	if origin == nil {
		return ""
	}
	// Extract from author URI (agent://alice/work -> agent://alice)
	author := strings.TrimSpace(origin.Author)
	// Skip the scheme part (agent://)
	if idx := strings.Index(author, "://"); idx >= 0 {
		rest := author[idx+3:] // "alice/work"
		if slashIdx := strings.Index(rest, "/"); slashIdx > 0 {
			return author[:idx+3+slashIdx] // "agent://alice"
		}
	}
	return ""
}

// v0.5: get HD parent from message extensions
func getHDParentFromMessage(msg Message) string {
	if msg.Extensions == nil {
		return ""
	}
	if parent, ok := msg.Extensions["hd.parent"].(string); ok {
...
func ApplyWriterPolicyWithDelegations(index Index, project string, policy WriterPolicy, store DelegationStore) Index {
	...
	for _, bundle := range index.Bundles {
		...
		case "post":
			if !policy.acceptsOriginWithMessage(bundle.Message, bundle.Message.Kind, store) {
				continue
			}
