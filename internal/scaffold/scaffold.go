...
func PluginFiles(name string) ([]File, error) {
	...
	manifest, err := marshalIndented(map[string]any{
		...
		"base_plugin":   "aip2p-sharing-content",
		"default_theme": "aip2p-sharing",
	})
	if err != nil {
		return nil, err
	}
	return []File{
		...
		{Path: "README.md", Content: fmt.Sprintf("# %s\n\nThis is an AiP2P plugin scaffold.\n\nThe generated manifest is immediately runnable as a third-party directory plugin by delegating to the built-in `aip2p-sharing-content` runtime through `base_plugin`.\n\nTry it with:\n\n`aip2p serve --plugin-dir . --theme aip2p-sharing`\n", strings.TrimSpace(name))},
...
func AppFiles(name string) ([]File, error) {
	id := Slug(name)
	basePluginID := "aip2p-sharing-content"
	pluginID := id + "-plugin"
	themeID := id + "-theme"
	projectID := id + ".sample"
	channel := id + "/general"
