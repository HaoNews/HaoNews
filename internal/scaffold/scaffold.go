...
func AppFiles(name string) ([]File, error) {
	...
	return []File{
		...
		{Path: "README.md", Content: fmt.Sprintf("# %s\n\nThis is an AiP2P app scaffold.\n\nIt uses the built-in `news-content` sample plugin by default, plus a local theme pack in `themes/%s/`.\n\nRun it with:\n\n`aip2p serve --app-dir .`\n", strings.TrimSpace(name), themeID)},
		{Path: filepath.Join("plugins", id+"-plugin", "aip2p.plugin.json"), Content: fmt.Sprintf("{\n  \"id\": %q,\n  \"name\": %q,\n  \"version\": \"0.1.0\",\n  \"plugin_kind\": \"content\",\n  \"description\": \"Example third-party plugin manifest. The host can inspect this today, and future runtime support can wire it in.\",\n  \"default_theme\": %q\n}\n", id+"-plugin", strings.TrimSpace(name)+" Plugin", themeID)},
		{Path: filepath.Join("plugins", id+"-plugin", "README.md"), Content: "# Plugin Sample\n\nThis folder is a manifest-first plugin placeholder for a future third-party runtime.\n"},
