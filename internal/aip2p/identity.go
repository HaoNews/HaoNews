...
type AgentIdentity struct {
	...
	Mnemonic       string `json:"mnemonic,omitempty"`        // BIP39 mnemonic (plaintext, cleared when encrypted)
	MasterPubKey   string `json:"master_pubkey,omitempty"`   // Master public key
	DerivationPath string `json:"derivation_path,omitempty"` // e.g., "m/0/0"
	Parent         string `json:"parent,omitempty"`          // Parent author URI

	// Encryption extensions
	MnemonicEncrypted string `json:"mnemonic_encrypted,omitempty"` // Base64 encrypted mnemonic
	EncryptionSalt    string `json:"encryption_salt,omitempty"`    // Base64 salt
	EncryptionMethod  string `json:"encryption_method,omitempty"`  // e.g., "aes-256-gcm+argon2id"
...
func BuildSignedOrigin(msg Message, identity AgentIdentity) (*MessageOrigin, error) {
	...
	// Add HD extensions to message if identity is HD-enabled
	if identity.HDEnabled {
		if msg.Extensions == nil {
			msg.Extensions = make(map[string]any)
		}
		if identity.Parent != "" {
			msg.Extensions["hd.parent"] = identity.Parent
		}
		if identity.MasterPubKey != "" {
			msg.Extensions["hd.parent_pubkey"] = "ed25519:" + identity.MasterPubKey
		}
		if identity.DerivationPath != "" {
			msg.Extensions["hd.path"] = identity.DerivationPath
		}
	}
...
// NewHDIdentity creates a new HD master identity with mnemonic
func NewHDIdentity(agentID, author string, createdAt time.Time) (AgentIdentity, error) {
	agentID = strings.TrimSpace(agentID)
	author = strings.TrimSpace(author)
	if agentID == "" {
		return AgentIdentity{}, errors.New("agent_id is required")
	...
	return AgentIdentity{
		...
		HDEnabled:      true,
		Mnemonic:       mnemonic,
		MasterPubKey:   hex.EncodeToString(masterKey.PublicKey()),
		DerivationPath: "m/0",
	}, nil
}

// DeriveChildIdentity derives a child identity from a master identity
func DeriveChildIdentity(masterIdentity AgentIdentity, childPath string) (AgentIdentity, error) {
	if !masterIdentity.HDEnabled {
		return AgentIdentity{}, errors.New("parent identity is not HD-enabled")
	}
	if masterIdentity.Mnemonic == "" {
		return AgentIdentity{}, errors.New("parent identity has no mnemonic")
	}

	// Derive child key
	seed, err := MnemonicToSeed(masterIdentity.Mnemonic, "")
	if err != nil {
		return AgentIdentity{}, err
	}
	childKey, err := DerivePathFromSeed(seed, childPath)
	if err != nil {
		return AgentIdentity{}, err
	}
	...
	// Build child author URI
	childAuthor := masterIdentity.Author
	if childAuthor == "" {
		childAuthor = "agent://" + masterIdentity.AgentID
	}
	// Extract path suffix from derivation path
	// e.g., m/0/0 -> /work (requires mapping)
	// For now, use simple numeric suffix
	pathParts := strings.Split(childPath, "/")
	if len(pathParts) > 2 {
		childAuthor += "/child-" + strings.Join(pathParts[2:], "-")
	}

	return AgentIdentity{
		AgentID:        masterIdentity.AgentID + "-" + strings.ReplaceAll(childPath, "/", "-"),
		Author:         childAuthor,
		KeyType:        KeyTypeEd25519,
		PublicKey:      hex.EncodeToString(publicKey),
		PrivateKey:     hex.EncodeToString(privateKey),
		...
		Parent:         masterIdentity.Author,
		MasterPubKey:   masterIdentity.MasterPubKey,
	}, nil
}

// LoadMasterIdentity loads a master identity by author URI from the registry
func LoadMasterIdentity(author string) (*AgentIdentity, error) {
	// Extract base author (remove path after scheme)
	baseAuthor := author
	if idx := strings.Index(author, "://"); idx >= 0 {
		rest := author[idx+3:]
		if slashIdx := strings.Index(rest, "/"); slashIdx > 0 {
			baseAuthor = author[:idx+3+slashIdx]
		}
	}
	...
	registryPath := home + "/.aip2p/identity_registry.json"
	registry, err := LoadIdentityRegistry(registryPath)
	if err != nil {
		return nil, err
	}

	entry, ok := registry.Get(baseAuthor)
	if !ok {
		return nil, fmt.Errorf("identity %s not found in registry", baseAuthor)
	}
