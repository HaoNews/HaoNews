...
func TestValidatePluginManifest(t *testing.T) {
	manifest := apphost.PluginManifest{
		ID:           "sample-content",
		Name:         "Sample Content",
		BasePlugin:   "aip2p-sharing-content",
		DefaultTheme: "aip2p-sharing",
	}
	report, err := ValidatePluginManifest(manifest, stubResolver{})
	if err != nil {
		t.Fatalf("validate plugin manifest: %v", err)
	}
	if report.Base == nil || report.Base.ID != "aip2p-sharing-content" {
...
func TestValidateAppBundle(t *testing.T) {
	...
	registry.MustRegisterTheme(themeWithManifest(apphost.ThemeManifest{
		ID:               "aip2p-sharing",
		Name:             "Default News",
		SupportedPlugins: []string{"aip2p-sharing-content"},
		RequiredPlugins:  []string{"aip2p-sharing-content"},
	}))
	...
	if len(report.Plugins) != 1 || report.Plugins[0].Base == nil || report.Plugins[0].Base.ID != "aip2p-sharing-content" {
