# AiP2P Theme Manifest 设计草案

## 1. 文档目的

这份文档定义未来 `aip2p` theme 的 manifest 方向。
...
3. theme manifest 可以声明支持哪些插件，但仍然和插件保持分离
4. theme config 只作用于显示层
5. 当前 `aip2p-news` UI 已经收敛为 `default-news` theme 的实现来源，后续继续以 manifest 驱动结构演进
