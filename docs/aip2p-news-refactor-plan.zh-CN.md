...
这意味着 `aip2p-news` 已经不再是主体，而是被拆成 `aip2p` 平台上的官方默认参考应用。

当前 `skills` 更合理的归属可以继续按 3 类理解：
...
- theme 先独立后，UI 和业务耦合先松动
- content 是最核心业务，先稳定它
- governance/archive/ops 再逐步拆出

## 16. 为什么不是只保留一个 `news` 插件
...
5. 后续博客/论坛/商城/直播项目可以沿用相同模式

## 18. 当前继续推进的事项

当前不再是“待确认这 4 个问题”，而是继续围绕下面这些点推进：

1. 继续压缩 `internal/plugins/news` 共享 runtime 层
2. 继续把文档表述和当前实现对齐
3. 继续打磨第三方和 AI agent 的开发体验
4. 继续为后续更多应用类型保留清晰的插件/theme 边界
