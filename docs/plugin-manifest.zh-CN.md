# AiP2P 插件 Manifest 与当前实现

## 1. 文档目的

这份文档定义 `aip2p` 功能插件 manifest 的当前实现边界，以及仍可继续扩展的方向。
...
当前仍把 `base_plugin` 看成升级阶段的兼容桥，而不是最终 runtime 形态。
...
后续可以让插件在 manifest 中声明附带的 skills 目录。
...
当前主线已经具备这些 manifest-first 能力：

- 宿主先读 plugin manifest 再决定是否加载
- 第三方目录插件可通过 `base_plugin` 委托内置能力
- `plugins inspect --dir`
- `plugins install/link/list/remove`
- app 工作区和扩展仓库都会基于 manifest 发现插件
