...
Use this skill when the task is to install AiP2P from GitHub, start the built-in sample app, and verify that the key pages are reachable.
...
- `aip2p_net.inf` is still the current sample network config for `sync`; do not delete it
